...
class AC17CPABE(ABEnc):
    ...
    def keygen(self, pk, msk, attr_list):
        ...
        for t in range(self.assump_size):
            ...
            prod *= (g ** (sigma / a_t))       
            Kp.append(prod)
        # compute sk_theta for revocation
        Kp.append(g_k[self.assump_size]*(g ** (-sigma)) / g_x)
    ...
    def DK(self, pk, key, sk_tx):
        ...
        ht = self.group.hash(time, G1)
        rxPrime = self.group.random(ZR)
        key['Kp'][2] = key['Kp'][2] * sk_tx[0] * ht**rxPrime 
        sk_0_4 = sk_tx[1] * pk['h']**rxPrime
        key['K_0'].append(sk_0_4)		
    ...
    def encrypt(self, pk, msg,  randomness, policy_str):
        ...
        M = pk['e_gh_kA'][0] # dummy, to simplify implementation      
        cPrime = Cp * M
        cPrimePrime = int.from_bytes(hashlib.sha512(str(M).encode()).digest(), byteorder='big') + msg # additional element to simplify implementation, negaligible impact


        return {'policy': policy, 'C_0': C_0, 'C': C, 'cPrime': cPrime, 'cPrimePrime': cPrimePrime}
    ...
    def decrypt(self, pk, ctxt, key, sk_tx):
        ...
        prod2_GT = prod2_GT * ctxt['cPrime'] * pair(ctxt['C_0'][3], key['K_0'][3])
       

        M = prod2_GT / prod1_GT
        msg = ctxt['cPrimePrime'] - int.from_bytes(hashlib.sha512(str(M).encode()).digest(), byteorder='big')
