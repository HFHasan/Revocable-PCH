...
def Adapt(cpabe,pk, sk_tx, ctxt, aes_data, iv, h_msg, h_msgPrime, r, key, rsaKey, etd_rsaKey, h_rsa, policy_str):
    g = pk['g']
    h = pk['h']
    group = cpabe.get_group_obj()
    
    # step 1 Verify rsach
    if (Verify(rsaKey, etd_rsaKey, h_msg, r, h_rsa) != 0): # failed
        return None
    
    # step 2
    data = cpabe.decrypt(pk, ctxt, key, sk_tx)
    ...
    # step 3
    randomness = H3(cpabe, str(rPrime))
    ...
    # AES decryption to get d2, ephemeral trapdoor
    cipher = AES.new(aesKey, AES.MODE_CBC, iv)
...
def main():
    ...
    _r = Adapt(cpabe, pk, sk_tx, ctxt, aes_data, iv, h_msg, h_msgPrime, r, key, rsaKey, etd_rsaKey, h_rsa, policy_str)
